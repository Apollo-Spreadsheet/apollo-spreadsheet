...
export const ApolloSpreadSheet = forwardRef(
	(props: Props, componentRef: React.Ref<ApolloSpreadSheetRef>) => {
		...
		console.error(fixedColumnWidths.current)
							...
							>
								{({ scrollTop, scrollLeft, isScrolling, gridRef, headerRef, height }) => {
									console.log({
										...
										containerWidth,
										scrollbarSize,
										mathMaxOfScroll: Math.max(0, scrollbarSize ?? 0),
										totalSize: fixedColumnWidths.current.totalSize,
									})
									return (
										<>
											<ColumnGrid
