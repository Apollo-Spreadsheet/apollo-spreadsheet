...
const GridWrapper = forwardRef((props: Props, componentRef: any) => {
  ...
  const onCellDoubleClick = useCallback(
    (e, rowIndex, colIndex, defaultValue) => {
      e.preventDefault();
    ...
    [props.rows, activeEditor]
  );

  const renderCell = useCallback(
    ({ style, cell, ref, rowIndex, columnIndex, isScrolling }) => {
      const { children } = cell;
      const isSelected =
        ...
        <div
          ...
          style={{
            ...
            boxSizing: "border-box",
              /** @todo Review if pointerEvents affect performance with isScrolling **/
            pointerEvents: (isScrolling || cell?.dummy) ? 'none' : 'auto',
            ...style,
          }}
          onClick={(event) => {
              if (cell?.dummy){
          ...
          onDoubleClick={(e) => {
            ...
            onCellDoubleClick(e, rowIndex, columnIndex, children);
          }}
          ref={ref}
        >
    ...
    [props.coords, props.theme, props.onCellClick, props.headers]
  );

  const cellRenderer = useCallback(
    (args) => {
      const cell = props.rows[args.rowIndex]?.[args.columnIndex];
    ...
    [props.rows, props.coords, props.theme]
  );
  ...
  return (
    <>
      ...
      {activeEditor &&
        createPortal(
          activeEditor.editor,
          document.getElementById("grid-container") as any
        )}
    </>
  );
});
